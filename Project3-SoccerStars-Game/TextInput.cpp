...
TextInputWindow::TextInputWindow(int _width, int _height,std::string field_name, Window *window)
    : width(_width), height(_height), img_background("Assets/logo.jpg"),
      inputString((_width - text_input_width) / 2,
                  (_height - text_input_height) / 2, 30,field_name) {
  win = window;
  enterPressed = false;
}
